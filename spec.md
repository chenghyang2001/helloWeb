...
### 功能 1：問候訊息
- 頁面顯示文字：`hello 楊政憲 楊政憲 楊政憲`
- 三個「楊政憲」以不同顏色顯示：第一個為紫藍色（`#667eea`），第二個為深紫色（`#764ba2`），第三個為橙紅色（`#e53e3e`）
- 置於頁面頂部的 `<h1>` 標籤內</old>
</spec_replace>

<spec_replace>
<old>- 標籤右側顯示版本號與發佈時間戳記，格式為 `v.001 | YYYY-MM-DD HH:MM:SS`（含時分秒）</old>
<new>- 標籤右側顯示版本號與發佈時間戳記，格式為 `v.002 | YYYY-MM-DD HH:MM:SS`（含時分秒）
